...
public class MyMusicPlayer extends ListenerAdapter {
	...
	RequiredPermissionLevel user = RequiredPermissionLevel.USER;
	RequiredPermissionLevel admin = RequiredPermissionLevel.ADMIN;
	RequiredPermissionLevel dev = RequiredPermissionLevel.DEV;
	...
	private void buildCommands() {
		...
		c = new Command(name, user) {
			public void response(PrivateMessageReceivedEvent e) {
				...
				LinkedList<AudioTrack> userSongList = trackScheduler.getUserSongs(u.getId());
				ListIterator<AudioTrack> userSongs = userSongList.listIterator(0);
				
				if(demo.getJda().getGuildById(serverId).getAudioManager().isConnected()) {
					e.getChannel().sendMessage("queue currently contains " + trackScheduler.currentQueueSize() + " songs. you have " 
							+ userSongList.size() + " song(s) in queue").queue();
...
//		commandNames.add(name);
		...
		c = new Command(name, admin) {
			public void response(PrivateMessageReceivedEvent e) {
				...
				String serverId="";
				relevantInfo = getTargetInfo(e);
				String[] words = e.getMessage().getContentDisplay().split(" ");
		...
		//exit functionality is provided by mylistener. keeping this for posterity
//		name = "!exit";
...
//		commandNames.add(name);
	...
	private void queueSong(PrivateMessageReceivedEvent e, String serverId) {
		...
		String songUrl = e.getMessage().getContentDisplay().split(" ")[1];
	...
	private boolean hasPermission(String userId, RequiredPermissionLevel permission) {
		if(permission == RequiredPermissionLevel.ADMIN) {
			return (this.adminDatabase.contains(userId) || this.devDatabase.contains(userId));
		} else if(permission == RequiredPermissionLevel.DEV) {
